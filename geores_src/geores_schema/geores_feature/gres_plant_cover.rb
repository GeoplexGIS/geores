...
class GRES_PlantCover < GRES_CityObject
  def initialize
    super()
  end
   ...
   retstring << "<veg:PlantCover gml:id=\"" + @gmlid + "\">\n"
    ...
    @simpleCityObjectAttributes.each { |att|
     ...
     if(@lod1MultiSurface.length > 0)
        retstring  << "<veg:lod1MultiSurface>\n"
      ...
      if(@lod2MultiSurface.length > 0)
        retstring  << "<veg:lod2MultiSurface>\n"
        retstring << "<gml:MultiSurface>\n"
      ...
      if(@lod3MultiSurface.length > 0)
        retstring  << "<veg:lod3MultiSurface>\n"
        retstring << "<gml:MultiSurface>\n"
      ...
      if(@lod4MultiSurface.length > 0)
        retstring  << "<veg:lod4MultiSurface>\n"
        retstring << "<gml:MultiSurface>\n"
