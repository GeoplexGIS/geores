...
class GRES_CityFurniture < GRES_CityObject
  ...
  attr_reader :implicitgeometriesLod1, :implicitgeometriesLod2, :implicitgeometriesLod3,  :implicitgeometriesLod4
    ...
    if(name.index("lod4") != nil)
     ...
     if(isrefobject == true)
       @implicitgeometriesLod1.each { |implGeom|
         ...
         GRES_CGMLDebugger.writedebugstring("CityFurniture Erzeuge neues implizites Referenzobjekt mit Namen " + implgroup.name + " und id: " + implGeom.gmlid + "\n")
         if(implGeom == nil or implGeom.geometries == nil)
           GRES_CGMLDebugger.writedebugstring("CityFurniture Fehler:  implizite Geometrie oder beinhaltete Geometrien null \n")
         end
         implGeom.geometries.each { |geo|
           geo.build(implgroup, appearances, citygmlloader, parents, implGeom.gmlid, "lod1", layer_creator.cityfurnitures, true, false)
         }
         GRES_CGMLDebugger.writedebugstring("CityFurniture Geometrien fuer implizites Objekt erstellt.\n")
         ...
         GRES_CGMLDebugger.writedebugstring("CityFurniture 4x4 Transformation des Objektes " + implGeom.trafo.to_a.to_s + "\n")
          GRES_CGMLDebugger.writedebugstring("CityFurniture Punkt Transformation des Objektes " + implGeom.trafopoint.to_a.to_s + "\n")
          ...
          newgroup.layer = layer_creator.cityfurnitures
       ...
       if(implrefobjects == nil)
         GRES_CGMLDebugger.writedebugstring("CityFurniture Implizite Referenzobjekte sind nil \n")
       end
       GRES_CGMLDebugger.writedebugstring("CityFurniture Erzeuge neue implizite Geometrie als Kopie fuer Objekt mit Namen " + @theinternalname + "\n")

        @implicitgeometriesLod1.each { |implGeom|
           GRES_CGMLDebugger.writedebugstring("CityFurniture Suche Referenzobjekt fuer" + implGeom.xlink +  "\n")
              implrefobjects.each { |implref|
                implref.implicitgeometriesLod1.each{|implrefImplGeom|
                  if(implrefImplGeom.gmlid == implGeom.xlink)
                   GRES_CGMLDebugger.writedebugstring("CityFurniture Referenzkomponente gefunden :" + implGeom.xlink +  "\n")
         ...
         if(thecompInst != nil)
           GRES_CGMLDebugger.writedebugstring("CityFurniture ComponentInstance gefunden! Erzeuge neue Instanz\n")
    ...
    def buildImplicitGeometryLoD2(entities, appearances, citygmlloader, parents, isrefobject)
     ...
     if(isrefobject == true)
       @implicitgeometriesLod2.each { |implGeom|
         ...
         implGeom.geometries.each { |geo|
           geo.build(implgroup, appearances, citygmlloader, parents, implGeom.gmlid, "lod2", layer_creator.cityfurnitures, true, false)
         }
         GRES_CGMLDebugger.writedebugstring("CityFurniture Geometrien fuer implizites Objekt erstellt.\n")
         comp_inst = implgroup.to_component
         comp_instdict = comp_inst.attribute_dictionary("groupatts", true)
    ...
    def buildImplicitGeometryLoD3(entities, appearances, citygmlloader, parents, isrefobject)
     ...
     if(isrefobject == true)
       @implicitgeometriesLod3.each { |implGeom|
         ...
         implGeom.geometries.each { |geo|
           geo.build(implgroup, appearances, citygmlloader, parents, implGeom.gmlid, "lod3", layer_creator.cityfurnitures, true, false)
         }
         GRES_CGMLDebugger.writedebugstring("CityFurniture Geometrien fuer implizites Objekt erstellt.\n")
         comp_inst = implgroup.to_component
         comp_instdict = comp_inst.attribute_dictionary("groupatts", true)
   ...
   def buildImplicitGeometryLoD4(entities, appearances, citygmlloader, parents, isrefobject)
     ...
     if(isrefobject == true)
       @implicitgeometriesLod4.each { |implGeom|
         ...
         implGeom.geometries.each { |geo|
           geo.build(implgroup, appearances, citygmlloader, parents, implGeom.gmlid, "lod4", layer_creator.cityfurnitures, true, false)
         }
         GRES_CGMLDebugger.writedebugstring("CityFurniture Geometrien fuer implizites Objekt erstellt.\n")
         comp_inst = implgroup.to_component
         comp_instdict = comp_inst.attribute_dictionary("groupatts", true)
       ...
       GRES_CGMLDebugger.writedebugstring("CityFurniture Erzeuge neue implizite Geometrie als Kopie fuer Objekt mit Namen " + @theinternalname + "\n")
        ...
        @implicitgeometriesLod4.each { |implGeom|
           GRES_CGMLDebugger.writedebugstring("CityFurniture Suche Referenzobjekt fuer" + implGeom.xlink +  "\n")
              implrefobjects.each { |implref|
                implref.implicitgeometriesLod4.each{|implrefImplGeom|
                  if(implrefImplGeom.gmlid == implGeom.xlink)
                   GRES_CGMLDebugger.writedebugstring("CityFurniture Referenzkomponente gefunden :" + implGeom.xlink +  "\n")
                   thecompInst = implrefImplGeom.compInst
                end
         ...
         if(thecompInst != nil)
           GRES_CGMLDebugger.writedebugstring("CityFurniture ComponentInstance gefunden! Erzeuge neue Instanz\n")
           newgroup = entities.add_instance(thecompInst.definition, thecompInst.transformation)
           newgroup.name= @theinternalname +"@implicitReferenceLoD4"
            GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: 4x4 Transformation des Objektes " + implGeom.trafo.to_a.to_s + "\n")
           GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: Punkt Transformation des Objektes " + implGeom.trafopoint.to_a.to_s + "\n")
           newgroup.transform! implGeom.trafo
                   po = implGeom.trafopoint
   ...
   retstring << "<frn:CityFurniture gml:id=\"" + @gmlid + "\">\n"
    ...
    @simpleCityObjectAttributes.each { |att|
     ...
     if(@lod1MultiSurface.length > 0)
      ...
      if(@implicitgeometriesLod1.length  > 0)
         retstring  << "<frn:lod1ImplicitRepresentation>\n"
         @implicitgeometriesLod1.each { |implGeom|
        ...
        if(@implicitgeometriesLod2.length  > 0)
         retstring  << "<frn:lod2ImplicitRepresentation>\n"
         @implicitgeometriesLod2.each { |implGeom|
            retstring << implGeom.writeToCityGML
         }
        retstring  << "</frn:lod2ImplicitRepresentation>\n"
        ...
        if(@implicitgeometriesLod3.length  > 0)
         retstring  << "<frn:lod3ImplicitRepresentation>\n"
         @implicitgeometriesLod3.each { |implGeom|
            retstring << implGeom.writeToCityGML
         }
        retstring  << "</frn:lod3ImplicitRepresentation>\n"
        ...
        if(@implicitgeometriesLod4.length  > 0)
         retstring  << "<frn:lod4ImplicitRepresentation>\n"
         @implicitgeometriesLod4.each { |implGeom|
            retstring << implGeom.writeToCityGML
         }
        retstring  << "</frn:lod4ImplicitRepresentation>\n"
