...
class GRES_SolitaryVegetationObject < GRES_CityObject
   ...
   def buildgeometries(entities, appearances, citygmlloader, parentnames)
      parents = Array.new()
       ...
       if(@isImplicitReferenceObject == true)
        GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: Erzeuge implizite Geometrien als Refernz fuer" + @theinternalname +"\n")
       ...
       return
        ...
        if(@isImplicitObject == true)
          GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: Erzeuge implizite Geometrien als Kopie fuer" + @theinternalname +"\n")
   ...
   def buildImplicitGeometryLoD1(entities, appearances, citygmlloader, parents, isrefobject)
     ...
     if(isrefobject == true)
       @implicitgeometriesLod1.each { |implGeom|
         ...
         GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: Erzeuge neues implizites Referenzobjekt mit Namen " + implgroup.name + " und id: " + implGeom.gmlid + "\n")
         if(implGeom == nil or implGeom.geometries == nil)
           GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject Fehler:  implizite Geometrie oder beinhaltete Geometrien null \n")
         end
         implGeom.geometries.each { |geo|
           geo.build(implgroup, appearances, citygmlloader, parents, implGeom.gmlid, "lod1", layer_creator.solveg, true, false)
         }
         GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: Geometrien fuer implizites Objekt erstellt.\n")
         ...
         GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: 4x4 Transformation des Objektes " + implGeom.trafo.to_a.to_s + "\n")
          ...
          newgroup.layer = layer_creator.solveg
       ...
       if(implrefobjects == nil)
         GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject:Fehler: Implizite Referenzobjekte sind nil \n")
       end
       GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: Erzeuge neue implizite Geometrie als Kopie fuer Objekt mit Namen " + @theinternalname + "\n")

        @implicitgeometriesLod1.each { |implGeom|
           GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: Suche Referenzobjekt fuer" + implGeom.xlink +  "\n")
              implrefobjects.each { |implref|
                implref.implicitgeometriesLod1.each{|implrefImplGeom|
                  if(implrefImplGeom.gmlid == implGeom.xlink)
                   GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: Referenzkomponente gefunden :" + implGeom.xlink +  "\n")
         ...
         if(thecompInst != nil)
           GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: ComponentInstance gefunden! Erzeuge neue Instanz\n")
    ...
    def buildImplicitGeometryLoD2(entities, appearances, citygmlloader, parents, isrefobject)
     ...
     if(isrefobject == true)
       @implicitgeometriesLod2.each { |implGeom|
         ...
         implGeom.geometries.each { |geo|
           geo.build(implgroup, appearances, citygmlloader, parents, implGeom.gmlid, "lod2", layer_creator.solveg, true, false)
         }
         GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: Geometrien fuer implizites Objekt erstellt.\n")
         comp_inst = implgroup.to_component
          comp_instdict = comp_inst.attribute_dictionary("groupatts", true)
    ...
    def buildImplicitGeometryLoD3(entities, appearances, citygmlloader, parents, isrefobject)
     ...
     if(isrefobject == true)
       @implicitgeometriesLod3.each { |implGeom|
         ...
         implGeom.geometries.each { |geo|
           geo.build(implgroup, appearances, citygmlloader, parents, implGeom.gmlid, "lod3", layer_creator.solveg, true, false)
         }
         GRES_CGMLDebugger.writedebugstring("SolitaryVegetationObject: Geometrien fuer implizites Objekt erstellt.\n")
         comp_inst = implgroup.to_component
         comp_instdict = comp_inst.attribute_dictionary("groupatts", true)
...
def writeToCityGML isWFST, namespace
   ...
   retstring << "<veg:SolitaryVegetationObject gml:id=\"" + @gmlid + "\">\n"
    ...
    @simpleCityObjectAttributes.each { |att|
     ...
     if(@lod1MultiSurface.length > 0)
      ...
      if(@implicitgeometriesLod1.length  > 0)
         retstring  << "<veg:lod1ImplicitRepresentation>\n"
         @implicitgeometriesLod1.each { |implGeom|
        ...
        if(@implicitgeometriesLod2.length  > 0)
         retstring  << "<veg:lod2ImplicitRepresentation>\n"
         @implicitgeometriesLod2.each { |implGeom|
            retstring << implGeom.writeToCityGML
         }
        retstring  << "</veg:lod2ImplicitRepresentation>\n"
        ...
        if(@implicitgeometriesLod3.length  > 0)
         retstring  << "<veg:lod3ImplicitRepresentation>\n"
         @implicitgeometriesLod3.each { |implGeom|
            retstring << implGeom.writeToCityGML
         }
        retstring  << "</veg:lod3ImplicitRepresentation>\n"
        ...
        if(@implicitgeometriesLod4.length  > 0)
         retstring  << "<veg:lod4ImplicitRepresentation>\n"
         @implicitgeometriesLod4.each { |implGeom|
            retstring << implGeom.writeToCityGML
         }
        retstring  << "</veg:lod4ImplicitRepresentation>\n"
       ...
       retstring << "</veg:SolitaryVegetationObject>\n"
       retstring << "</core:cityObjectMember>\n"
